...
function paginate(selectedPage, totalPages) {
  ...
  for (let currentPage = 1; currentPage <= totalPages; currentPage++) {
    ...
    const secondAndPenultimatePages = currentPage == 2 || currentPage == totalPages -1
    const pagesAfterSelectedPage = currentPage <= selectedPage + 1
    const pagesBeforeSelectedPage = currentPage >= selectedPage - 1

    if(firstAndLastPages || secondAndPenultimatePages || pagesBeforeSelectedPage && pagesAfterSelectedPage) {
